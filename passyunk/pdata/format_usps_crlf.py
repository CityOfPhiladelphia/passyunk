...
def main(username, password): 
    ...
    with usps_epf.Usps_Epf(username, password) as epf: 
        print(epf.get_list())
        ...
        if newest == None: 
            ...
            sys.exit()
    io_bytes = io.BytesIO(newest.content)
    folder = newest.filename.removesuffix('.tar')
    with tarfile.open('zip4byst.tar', 'w') as tar: 
        info = tarfile.TarInfo(name='zip4byst')
        tar.addfile(tarinfo=info, fileobj=io_bytes)
